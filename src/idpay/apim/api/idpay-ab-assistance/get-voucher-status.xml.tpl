...
<policies>
  <inbound>
    ...
    <set-variable name="fiscalCode" value="@(((JObject)context.Variables.GetValueOrDefault("requestBody"))?["fiscalCode"]?.ToString())" />
    ...
    <choose>
      <when condition="@(!context.Variables.ContainsKey("pdv_token") || string.IsNullOrEmpty((string)context.Variables["pdv_token"]))">
        <return-response>
          <set-status code="400" reason="Tokenization Failed" />
          <set-body>{
    ...
    <rewrite-uri template="@("/idpay/assistance/vouchers/status/68dd003ccce8c534d1da22bc/" + (string)context.Variables["tokenPDV"])" />
