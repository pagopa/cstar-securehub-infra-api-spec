          ...
          content:
            application/json:
              schema:
                $ref: '#/components/schemas/WalletErrorDTO'
              example:
                code: 'WALLET_ENROLL_IBAN_NOT_ALLOWED'
                message: 'It is not possible to enroll an iban for a discount type initiative'
          headers:
            Access-Control-Allow-Origin:
              $ref: '#/components/headers/Access-Control-Allow-Origin'
            RateLimit-Limit:
  ...
  /support:
      post:
        tags:
          - support
        summary: >
          ENG: Build a Zendesk JWT and return redirect URL IT: Genera un JWT per
          Zendesk e restituisce la URL di redirect
        description: >
          ENG: Generates a JWT token for Zendesk SSO based on user data. IT:
          Genera un token JWT per SSO Zendesk a partire dai dati utente.
        operationId: buildZendeskJwt
        security:
          - bearerAuth: []
        parameters:
          - $ref: '#/components/parameters/ApiVersionHeader'
        ...
        responses:
          ...
          '400':
            ...
            content:
              application/json:
                schema:
                  $ref: '#/components/schemas/SupportErrorDTO'
                examples:
                  invalidEmail:
                    value:
  ...
  schemas:
    ...
    SupportRequestDTO:
      ...
      properties:
        email:
          type: string
          format: email
          description: 'ENG: User email (required) - IT: Email utente (obbligatoria)'
          minLength: 5
          maxLength: 254
        firstName:
          type: string
    ...
    SupportResponseDTO:
          ...
          properties:
            jwt:
              type: string
              description: 'ENG: Generated JWT - IT: JWT generato'
            returnTo:
              type: string
              format: uri
              description: >
