resource "azurerm_api_management_product" "this" {
  for_each = {
    for k, v in local.products : k => v
    if contains(try(v.env_enabled, [var.env_short]), var.env_short)
  }
...
resource "azurerm_api_management_product_policy" "this" {
  for_each = {
    ...
    if contains(keys(product), "policy") && contains(try(product.env_enabled, [var.env_short]), var.env_short)
  }
